...
function AddPhoto() {
    ...
    const onSubmit = async (data) => {
        const result = await uploadPhoto({ ...data, image })
        navigate('/');
    }
    ...
    return (
        <div className='wrapper'>
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className='inner'>
                    <h1>Upload Photo</h1>
                    <label htmlFor="file" className="custom-file-input">Choose Image</label>
                    <input id="file" accept="image/*" type="file" onChange={convertToBase64} />
                    {image && <img width={200} height={200} src={image} alt="uploaded image" />}
                    <div className='space'>
                    ...
                    <button disabled={!image} className='btn'>Submit</button>
                </div>
            </form>
        </div>
    )
